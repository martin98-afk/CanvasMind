...
from app.widgets.side_dock_area.plugins.sqlite_database.db_manager import DatabaseManager
...
class SessionStore:
    ...
    def save_memory(self, memory: Dict, canvas_id: str = "default") -> bool:
        ...
        memory_id = memory.get("memory_id") or memory.get("id") or datetime.now().strftime("%Y%m%d%H%M%S%f")
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    def save_memories(self, memories: List[Dict], canvas_id: str = "default") -> bool:
        ...
        try:
            for memory in memories:
                self.save_memory(memory, canvas_id)
            return True
        except Exception as e:
            logger.error(f"[SessionStore] 批量保存记忆失败: {e}")
            return False

    def load_memories(self, canvas_id: str = "default", limit: int = 100) -> List[Dict]:
        """加载长期记忆列表"""
        if not self.is_initialized:
            return []
