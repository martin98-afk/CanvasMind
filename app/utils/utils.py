...
def topological_sort(
    ...
    def topo_process(target_nodes, current_in_degrees):
        ...
        # 维护当前待处理节点的集合（用于在成环时选取视觉最靠前的节点）
        remaining_nodes = {n: get_node_visual_rank(n) for n in target_nodes}

        # 优先级队列：存放当前入度为 0 的节点
        ready_queue = []
        for n in target_nodes:
            if current_in_degrees[n] <= 0:
                heapq.heappush(ready_queue, (remaining_nodes[n], n))

        while len(order) < len(target_nodes):
            if not ready_queue:
                # 【核心修改：破环逻辑】
            ...
            # 正常执行拓扑提取
            rank, n = heapq.heappop(ready_queue)
            if n not in remaining_nodes:  # 防止破环逻辑重复添加
                continue
