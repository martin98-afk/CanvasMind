...
def execute_node(
    ...
    # 日志准备逻辑...
    is_log_node = (
        "StatusDynamicNode_" in node.model.type_ or "DYNAMIC_CODE" in node.model.type_
    )
    run_id = ""
    if is_log_node:
    ...
    try:
        ...
        elif "DYNAMIC_CODE" in node.model.type_:
            from app.nodes.dynamic_code_node import get_dynamic_component_class

            comp_cls = get_dynamic_component_class(node)
