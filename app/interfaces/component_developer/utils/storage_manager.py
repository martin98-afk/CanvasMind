...
class ComponentStorageManager:
    ...
    def _on_component_pasted(self, full_path):
        self._load_component(
            full_path=full_path, component=self.parent.component_tree._copied_component
        )
        # 源码路径修改
    ...
    def _create_new_component(self, component_info):
        ...
        template = self.parent.apply_component_info_to_code(
            self.parent.current_template_code, component_info
        )
    ...
    def _load_component(self, full_path=None, component=None, uuid=None):
        try:
            ...
            self.parent.requirements_edit.setText(
                getattr(component, "requirements", "").replace(",", "\n")
            )
                ...
                [
                    {
                        ...
                        "connection": getattr(
                            port, "connection", ConnectionType.SINGLE
                        ),
            ...
            if component is not None:
                source_code = self.extract_class_source_from_file(
                    source_file, component.__name__
                )
            ...
            if component is None:
                return
            QTimer.singleShot(
                300, lambda: self.parent.update_usage_table(component.uuid)
            )
    ...
    def _merge_requirements_from_extension(self):
        ...
        try:
            ...
            if new_reqs != ui_reqs:
                ...
                self.parent.requirements_edit.setText("\n".join(sorted_reqs))
                logger.info(
                    f"已自动合并 requirements.txt 中的依赖: {file_reqs - ui_reqs}"
                )
    ...
    def _save_component(self, delete_original_file: bool = True):
        ...
        try:
            ...
            self._save_component_to_file(
                category, name, code, self._current_component_file, delete_original_file
            )

            # 【核心逻辑】保存 Manifest
            comp_uuid = self._current_component_file.stem
            comp_info = {"name": name, "category": category, "description": description}
            source_file = (
                str(self._current_component_file)
                if self._current_component_file
                else ""
            )
            QTimer.singleShot(
                1000,
                lambda: self._update_extension_manifest(
                    comp_uuid, comp_info, source_file
                ),
            )
            ...
            if self._current_component_file:
                current_signature = {
                    ...
                    "properties": self.parent.property_editor.get_properties(
                        serialize=True
                    ),
                }
            ...
            QTimer.singleShot(
                1000,
                lambda: self._load_component(
                    full_path=f"{category}/{name}",
                    uuid=self._current_component_file.stem,
                ),
            )
    ...
    def save_component_by_full_path(self, full_path: str, new_code: str):
        try:
                ...
                return (
                    {
                        ...
                        "connection": getattr(
                            p, "connection", ConnectionType.SINGLE
                        ).name,
                    }
                    if hasattr(p, "name")
                    else p
            ...
            sig = {
                ...
                "outputs": [
                    {"name": p.name, "label": p.label, "type": p.type.name}
                    for p in current_signature["outputs"]
                ],
                "properties": {
                    k: serialize_property(v)
                    for k, v in current_signature["properties"].items()
                },
            }
            ComponentHistoryManager.save_history(
            ...
            if hasattr(comp_obj, "uuid"):
                ...
                src_file = str(source_file) if source_file else ""
                QTimer.singleShot(
                    1000,
                    lambda: self._update_extension_manifest(
                        comp_obj.uuid, info, src_file
                    ),
                )
    ...
    def _save_component_to_file(
        self, category, name, code, original_file_path=None, delete_original_file=True
    ):
        # 代码增加导入前缀
        ...
        if (
            original_file_path
            and components_dir / original_file_path.name == original_file_path
        ):
        ...
        if (
            ...
            and (components_dir / original_file_path.name).exists()
        ):
            ...
            delete_original_file
            and original_file_path
            and not (components_dir / original_file_path.name).exists()
        ):
    ...
    def _ensure_extension_environment(
        self, uuid_str, component_obj=None, source_file: str = ""
    ):
        ...
        if not target_dir.exists():
            shutil.copytree(
                self.extension_base_dir / "example_extension",
                target_dir,
                dirs_exist_ok=True,
            )
        ...
        info = {
            ...
            "description": getattr(component_obj, "description", "")
            if component_obj
            else "",
        }
        QTimer.singleShot(
            1000, lambda: self._update_extension_manifest(uuid_str, info, source_file)
        )
    ...
    def _update_extension_manifest(self, uuid_str, info: dict, source_file: str = ""):
        """生成或更新 manifest.json"""
        target_dir = self.extension_base_dir / uuid_str
        ...
        try:
            ...
            source_file_rel = str(
                Path("../../") / source_path.relative_to(Path(resource_path("app")))
            )
        except ValueError:
            # 如果不在 base_dir 下，保留原路径
        ...
        manifest = {
            ...
            "source_file": source_file_rel or existing_data.get("source_file", ""),
