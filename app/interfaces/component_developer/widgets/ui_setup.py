...
class ComponentDevelopUISetUp:
    ...
    def _cancel_edit(self):
        w = MessageBox("确认", "确定要取消编辑吗？未保存的更改将丢失。", self.parent.window())
        if w.exec():
    ...
    def _switch_template(self, template_name, template_code):
        ...
        MessageManager.success(f"已切换到模板: {template_name}", "", self.parent)
...
"""
        ...
        if not current_code.strip():
            MessageManager.warning("代码编辑器为空，无法运行！", "", self.parent)
            return
        ...
        if current_console:
            ...
            MessageManager.error("当前控制台未启动或无 kernel 客户端！", "", self.parent)
