...
class EnvironmentCardView(SimpleCardWidget):
    ...
    def __init__(self, package_manager, title=None, parent=None):
        ...
        self.contentLayout = QHBoxLayout()  # 环境卡片少，用HBox或者Flow都行，这里用HBox固定显示
    ...
    def _update_env_cards(self):
        ...
        if self.package_manager.envCombo.count() == 0:
            add_card = StylishCard(get_icon("惊叹号"), self.tr("新建环境"), self.tr("无运行环境，请新建"), self)
        else:
            add_card = StylishCard(FluentIcon.ADD, self.tr("新建环境"), self.tr("创建新环境"), self)
        add_card.clicked.connect(self.addEnvSignal.emit)

        # 管理卡片
        manage_card = StylishCard(FluentIcon.SETTING, self.tr("环境管理"), self.tr("管理已有环境"), self)
