    ...
    def init_ui(self):
        ...
        if self.mode == "market":
            if self.status_code == "match":
                badge = QLabel("已就绪")
                badge.setStyleSheet("background: rgba(40, 167, 69, 0.1); color: #28a745; border: 1px solid #28a745; border-radius: 4px; padding: 2px 6px; font-size: 10px;")
                header.addWidget(badge)
            elif self.status_code == "diff":
                badge = QLabel("有更新")
                badge.setStyleSheet("background: rgba(255, 193, 7, 0.1); color: #ffc107; border: 1px solid #ffc107; border-radius: 4px; padding: 2px 6px; font-size: 10px;")
                header.addWidget(badge)
            elif self.status_code == "old":
                badge = QLabel("云端较旧")
                badge.setStyleSheet("background: rgba(108, 117, 125, 0.1); color: #6c757d; border: 1px solid #6c757d; border-radius: 4px; padding: 2px 6px; font-size: 10px;")
                header.addWidget(badge)
        elif self.is_linked:
            badge = QLabel("已同步")
            badge.setStyleSheet("background: rgba(31, 111, 235, 0.1); color: #1f6feb; border: 1px solid #1f6feb; border-radius: 4px; padding: 2px 6px; font-size: 10px;")
            header.addWidget(badge)
        ...
        # 第三行：作者与时间 (修复作者显示)
        meta = QHBoxLayout()
        creator = self.data.get('创建人') or self.data.get('creator') or self.data.get('author') or '未知'
        m_time = str(self.data.get('最后修改时间') or self.data.get('updated_at') or '---')
        ...
        ver_tag.setStyleSheet("background: #21262d; color: #8b949e; border-radius: 4px; padding: 2px 8px; font-size: 11px;")
