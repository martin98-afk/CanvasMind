        ...
        try:
            ...
            def generate_markdown(input: list, output: list):
                ...
                for i, inp in enumerate(input):
                    input_desc += (f"- 参数{i + 1}：{inp['custom_key']}; 参数格式：{inp['format']}; 参数参考样例输入：{inp['current_value']}; "
                                  f"所属组件名：{inp['node_name']}; 组件参数类型：{inp['type']}\n")
                output_desc = ""
                for i, out in enumerate(output):
                    output_desc += (f"- 输出{i + 1}：{out['custom_key']}; 输出格式：{out['format']}; 所属组件名：{out['node_name']};"
                                    f" 组件参数类型：{out['type']}\n")
