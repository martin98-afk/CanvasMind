...
- 操作类型:跳转节点,当你认为需要用户追溯到画布中实际节点时,严格按照以下格式引用:
-- 单个节点引用
[画布中存在的节点名称](jump)
-- 多个节点引用
[节点名称1,节点名称2,节点名称3...](jump)
...
在思考中、代码、流程图中可直接使用节点名称进行代指,只有在需要时使用此规范。
"""


NODE_CREATE_CONTEXT_NORMS = """## 组件引用规范

- 操作类型:创建节点,当你认为想推荐用户创建某节点时,严格按照以下格式引用:

[已在组件库的组件名称](create)

- 操作类型:创建未在组件库组件代码,当你想推荐用户创建的节点并没有在提供的组件信息清单中,严格按照以下格式引用:

[你想新添加的组件的具体名称](generate)
...
- 建议先生成 [模型评估](generate) 组件,其不在选择组件库中。

错误示例:
- [create](数据加载器):格式颠倒
- [数据加载器](create:node_123):多了参数
- [数据加载器](inspect):不支持的操作
- [模型训练](create):模型训练不在提供的组件中
- [数据加载器](generate):数据加载器在当前组件库中

在思考中、代码、流程图中可直接使用组件名称进行代指,只有在需要时使用此规范。
"""