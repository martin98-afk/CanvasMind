...
from app.interfaces.canvas_interaface.constants import (
...
class LLMContextProvider:
    def __init__(
        ...
        self.canvas_tools = CanvasTools(
            graph=self.graph, node_operations=self.node_operations, parent=self.parent
        )

    def get_canvas_tools_schema(self):
        return get_canvas_tools_schema()

    def get_canvas_tools_executor(self):
        return self.canvas_tools

    def _register_contexts(self):
        """注册所有支持的大模型上下文类型"""
        self.context_register.register(
            "画布节点图像", self.extract_graph_image, self.select_node_by_name
        )
        self.context_register.register(
            "全局变量", self.extract_var_info, lambda *args: None
        )
        self.context_register.register(
            "画布节点", self.extract_graph_info, self.select_node_by_name
        )
        self.context_register.register(
            "组件信息", self.get_component_info, self.parent.show_category_dialog
        )
    ...
    def _extract_graph_info(self, nodes=None):
        ...
        for node in nodes:
            ...
            custom_props = {
                k: v
                for k, v in node.model._custom_prop.items()
    ...
    def extract_graph_info(self):
        ...
        if len(selected_nodes) > 0:
            ...
            return (
                ...
                else f"节点: {selected_nodes[0].name()}",
                response.format(
    ...
    def get_component_info(self):
        ...
        for value in selected_components.values():
            inputs_str = (
                "; ".join(
                    [f"{item.label} ({item.type.value})" for item in value.inputs]
                )
                or "无"
            )
            outputs_str = (
                "; ".join(
                    [f"{item.label} ({item.type.value})" for item in value.outputs]
                )
                or "无"
            )
            props_str = (
            ...
            rows.append(
                f"| {name} | {category} | {description} | {inputs_str} | {outputs_str} | {props_str} |"
            )
        ...
        return (
            ...
            response.format(
                NODE_CREATE_CONTEXT_NORMS=NODE_CREATE_CONTEXT_NORMS,
