def get_llm_provider_templates():
    """从设置中加载已保存的LLM服务商配置"""
    ...
    try:
        ...
        saved = setting.llm_saved_providers.value or {}
        logger.info(f"[LLM Providers] 加载到的配置: {saved}")
        templates = {}
        for name, config in saved.items():
            if name in ("系统默认配置",):
                continue
            # 本地模型（如Ollama）不需要API_KEY，或者有API_KEY，或者有有效的API_URL
            api_key = config.get("API_KEY", "")
            api_url = config.get("API_URL", "")
            logger.info(
                f"[LLM Providers] 处理 {name}: api_key={api_key}, api_url={api_url}"
            )
            if api_key or api_url or "本地" in name:
                templates[name] = config.copy()
        logger.info(f"[LLM Providers] 最终模板: {templates}")
        return templates
    except Exception as e:
        logger.error(f"[LLM Providers] 加载失败: {e}")
        return {}
