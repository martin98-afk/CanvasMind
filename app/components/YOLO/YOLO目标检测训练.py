...
class Component(BaseComponent):
    ...
    name = "YOLO 目标检测训练"
    category = "YOLO/目标检测"
    description = "YOLO目标检测训练组件用于基于YOLOv8目标检测模型对标准YOLO格式数据集（含train/val图像与标签）进行训练。输入为数据集ZIP文件或目录及可选预训练模型文件，输出为训练好的模型文件（.pt）和包含预测结果的验证图像。参数包括模型类型、训练轮数、批量大小、图像尺寸、运行设备和早停策略等可配置项。"
    ...
    def run(self, params, inputs=None):
        ...
        self.logger.info(f"验证图像目录: {val_img_dir}")
        ...
        # 如果 data.yaml 不存在，创建一个
        if not data_yaml.exists():
            self.logger.info("data.yaml 不存在，自动创建...")
            # 获取类别列表（从 label 文件中提取）
            classes = set()
            ...
            class_list = sorted([str(c) for c in classes])
            
            # 创建 data.yaml
            with open(data_yaml, 'w') as f:
                f.write(f"path: {data_dir / params.dataset_name}\n")
        ...
        if not img_path.exists():
            for pattern in ["val_batch1_pred.jpg", "val_batch0_labels.jpg", "results.png"]:
...
if __name__ == "__main__":
    ...
    result = model.debug(
        ...
        global_vars={},
        node_id="test_detect_train_node",
