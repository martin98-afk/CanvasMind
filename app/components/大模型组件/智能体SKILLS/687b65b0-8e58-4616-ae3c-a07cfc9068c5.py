...
class AgentSkillsComponent(BaseComponent):
    ...
    inputs = [
        PortDefinition(name="input_data", label="用户输入",
                       type=ArgumentType.TEXT),
        PortDefinition(name="history", label="对话历史", type=ArgumentType.JSON),
        PortDefinition(name="selected_skills_detail",
                       label="选中技能详情", type=ArgumentType.JSON),
        PortDefinition(name="skill_registry", label="技能注册表",
                       type=ArgumentType.JSON, optional=True),
        PortDefinition(name="workspace_path", label="工作空间路径",
                       type=ArgumentType.TEXT, optional=True),
    ]
    ...
    outputs = [
        ...
        PortDefinition(name="executed_commands",
                       label="执行记录", type=ArgumentType.JSON),
        PortDefinition(name="skill_used", label="使用的技能",
                       type=ArgumentType.TEXT),
        PortDefinition(name="execution_status",
                       label="执行状态", type=ArgumentType.TEXT),
    ]
...
| `float` | 浮点数输入 |
| `int` | 整数输入 |
| `choice` | 下拉选择（必须带 choices）|
| `file` | 文件选择器 | 本地/远程文件路径，需配 `ext` |
...
subprocess""",
    ...
    def run(self, params, inputs):
        ...
        # === 模型客户端 ===
        model_cfg = params.model[1] if isinstance(
            params.model, (list, tuple)) and len(params.model) > 1 else {}
        ...
        while round_idx < int(params.max_command_rounds):
            ...
            round_idx += 1
            self.logger.info(
                f"🔄 [ROUND {round_idx}/{params.max_command_rounds}] 开始 | 消息数:{len(messages)}")
            ...
            if params.enable_ask_user and ask_user_count < 3:  # 最多问询 3 次
                ...
                if ask_request:
                    ...
                    if user_response is None:
                        ...
                        messages.append(
                            {"role": "user", "content": "用户取消了信息补充，请尝试其他方式或告知无法完成。"})
                        continue

                    # 将用户回复作为新的 user 消息
                    user_msg = f"【用户补充信息】\n" + \
                        "\n".join(
                            [f"{k}: {v}" for k, v in user_response.items()])
            ...
            exec_entry = {**skill_request, **cmd_result,
                          "round": round_idx, "duration": cmd_duration}
            ...
            if match:
                try:
                    ...
                    if not request.get("title") or not request.get("message"):
                        self.logger.warning(
                            f"⚠️ [ASK_PARSE] 问询请求缺少 title 或 message: {request}")
                        return None
                    # 确保 fields 是列表
    ...
    def _get_skill_workdir(self, skill_id, skill_registry, workspace):
        ...
        if file_path:
            ...
            if skill_md_path.exists():
                ...
                self.logger.debug(
                    f"✅ [WORKDIR] 方案 1: 从 file_path 推导 -> {skill_dir}")
        ...
        if workspace:
            ...
            if skill_dir.exists():
                self.logger.debug(
                    f"✅ [WORKDIR] 方案 2: 从 path 推导 -> {skill_dir}")
                return str(skill_dir)
            else:
            ...
            if content.startswith("---"):
                ...
                if match:
                    try:
                        ...
                        desc = meta.get(
                            "description", "无描述") if meta else "无描述"
                    except Exception as e:
                        self.logger.debug(
                            f"⚠️ frontmatter 解析跳过 {skill_id}: {e}")
    ...
    def _execute_command(self, command, workspace, timeout, allowed_cmds):
        ...
        if allowed_cmds:  # ✅ 空列表表示不限制，直接跳过
            ...
            if not any(cmd_prefix == prefix or command.startswith(prefix + " ") or command == prefix for prefix in allowed_cmds):
                self.logger.warning(
                    f"⚠️ [EXEC] 命令不在白名单中（但仍执行，因配置宽松）: `{cmd_prefix}`")
                # ✅ 改为警告而非拒绝，保持灵活性
                # 如果仍需严格拒绝，取消下面这行的注释：
        ...
        try:
            if is_windows:
                ...
                c_match = re.search(
                    r'python\s+-c\s+([\'"])(.*?)(?<!\\)\1', exec_command, re.DOTALL)
                if c_match:
                    code = c_match.group(2)
                    import tempfile
                    fd, temp_path = tempfile.mkstemp(
                        suffix='.py', dir=workspace or ".", text=True)
            ...
            env = os.environ.copy()
            if workspace:
                env["PYTHONPATH"] = str(Path(workspace)) + (os.pathsep + env.get(
                    "PYTHONPATH", "")) if env.get("PYTHONPATH") else str(Path(workspace))
            ...
            output = e.output.decode('utf-8', errors='replace') if e.output and isinstance(
                e.output, bytes) else (e.output if isinstance(e.output, str) else "")
            return {
                **result,
                "success": False,
            ...
            )[0] if exec_command.split() else exec_command
            return {
                **result,
                "success": False,
            ...
            self.logger.exception(
                f"❌ [EXEC] 执行异常：{type(e).__name__}: {str(e)}")
            return {
                **result,
                "success": False,
