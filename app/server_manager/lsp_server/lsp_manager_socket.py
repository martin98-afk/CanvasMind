...
class LspClientManager(QThread):
    ...
    def run(self):
        try:
            # 启动 pylsp
            cmd = [self.python_path, "-m", "pylsp", "--tcp", "--port", "31415"]
            kwargs = {}
            if platform.system() == "Windows":
                kwargs["creationflags"] = subprocess.CREATE_NO_WINDOW
            self.process = subprocess.Popen(
                cmd,
                stdin=subprocess.DEVNULL,  # pylsp 不再读 stdin
                stdout=subprocess.DEVNULL,  # pylsp 不再写 stdout（通信走 TCP）
            ...
            self.endpoint = JsonRpcEndpoint(sock.makefile('rb'), sock.makefile('wb'))
